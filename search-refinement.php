...
class SearchRefinement extends Form_Tags {
	private $query_var; //絞り込み用
	private $separater; //URL内で使用するセパレータ
	private $action_url; //絞り込み検索Form用
	private $post__in;	//(array)記事IDでの絞込み用
	private $post__not_in;  //(array)記事一覧で除外する記事
	public $searef_request = array(); //絞込みリクエスト内容を格納
	private $posts_where; //フィルター後の posts_where
	private $posts_join;
	public $searef_result; //結果の格納用
	private $selected_category;
	private $args; //search_refinement()の引数格納用。デバッグ的
	private $sub_query_metavalue; //絞り込みに直接利用するwhere分格納用
	private $title_mapping = array(); //metakeyをlabelに置き換えるためのリスト
	private $inputhiddens = array();
	private $id_count = 0;
		...
		foreach ( $_REQUEST as $req_key => $req_value ){
			if ( $req_key == $this->query_var && !empty($req_value) ) {
				...
				foreach ( $r as $value ){
					...
					if ( is_array($value) ){ continue; } //debug
	...
	function get_selection($args=''){
		...
		$posts_where = array(' WHERE 1=1 ');
		foreach ( (array)$this->posts_where as $w_key => $w ){
			if ( (string)$w_key != $r['meta_key'] . "%09" . $r['meta_sign'] ){
				$w = preg_replace('/^AND /', '', $w);
				foreach ( preg_split('/ AND /',$w) as $w1 ){
					if ( $w1 && !preg_match('/\.meta_key *=/', $w1) ){
						$posts_where[] = $w1;
					}
				}
			}
		...
		$where = $posts_where;
		if ( !is_search() ) {
			$where[] = sprintf("(%s.meta_key = '%s')", ${wpdb}->postmeta, $r['meta_key']); 
		}
		...
		if ( ( (is_home() && $is_home) || !is_archive() ) && $default_cat ){ 
			...
			foreach ( (array)$default_cat as $key => $value ){
				switch ($key){
					case 'cat':
						$value = (int)$value;
						$where[] = "tt.taxonomy = 'category' AND (tt.term_id = '{$value}' OR tt.parent = '{$value}')";
						break;
					case 'taxonomy':
						$where[] = "tt.taxonomy = '{$value}'";
						break;
					case 'term':
						//$where[] = " (t.slug = '{$value}' OR tt.parent = t.term_id)"; // DEBUG:再帰的にサブカテゴリを読みたい //
						break;
				}
			}
		}
		...
		$q_inner  = ' ' . join(' ', $inner);
		$q_where  = ' ' . join(' AND ', $where);
		$result = $wpdb->get_results( $q_select.$q_from.$q_inner.$q_where, ARRAY_N );
...
function searef_searchbox($args=''){
	...
	$id_count = $searchrefinement->get_id_count();
	?>
    ...
    <div class="search-refinement">
        ...
        <div class="search-refinement-selecter">
        	<img src="<?php echo site_url(); ?>/wp-admin/images/loading.gif" title="now Searching..." alt="now Searching..." class="nowsearching" style="display:none; float:left;" />
            ...
            <form name="choice" action="<?php echo (is_home() && !$is_home) ? get_bloginfo('url') : $searchrefinement->get_action_url($default_cat); ?>" method="GET">
				...
				<?php echo $wrap_tag_before; ?>
					...
					foreach ( $terms as $term ) :
						...
						$select_id = sprintf("s-%s-selection-%s", $id_count, $attr_id);
						printf($section_wrap_before, $attr_id);
							...
							<?php foreach ( $meta_values as $meta_sign => $meta_value ) : ?>
								<?php $select_id = ( $meta_sign == 'lt' ) ? $select_id . '-lt' : $select_id; ?>
				...
				<div id="s-<?php echo $id_count; ?>-submit"><input type="submit" value=" <?php _e('Search'); ?> " onclick="on_submit_search(this.form);return false;" /></div>
				<?php if ( $onchange ) : ?></noscript><?php endif; ?>
            </form>
